...
class ResNetDec(nn.Module):
    def __init__(
        ...
        if skip_connection==True:
            self.up_layers_channel_init = 256
            ...
            if 1 in self.skip_layers:
                inplanes = self.up_layers_channel_init + self.in_channels//4
            else:
            ...
            if 0 in self.skip_layers:
                inplanes = self.up_layers_channel_init//2 + self.in_channels//8
            else:
        ...
        self.conv1 = nn.Conv2d(self.up_layers_channel_init//8, self.up_layers_channel_init//16, kernel_size=3, stride=1, padding=1, bias=False)
